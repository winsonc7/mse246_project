...
logisticModel_Reg <- train(LoanStatus ~ 1 + ThirdPartyDollars + TermInMonths + GrossApproval + Term.Multiple + Same.State + In.CA + Is.ThirdParty + Missing.Interest 
                       + Refinance + Delta + Private.Sector + Premier + CORPORATION + INDIVIDUAL + MISSING + PARTNERSHIP  
                       + Unemployment.YR + Avg.Home.Price + GDP.Delta.YR + Log.S.P.Open + BorrState.Unemployment + ProjectState.Unemployment
                       + BorrState.Income + Missing.Borr.Income + Missing.Proj.Income + BorrState.GDP + ProjState.GDP + Missing.Borr.GDP
                       + Missing.Proj.GDP + BorrState.Vacancy + ProjectState.Vacancy
                       , data = df, method = "glmnet",tuneGrid = data.frame(alpha=seq(0.1,0.8,0.05),
                                                                            lambda=seq(0.1,0.8,0.05)))

logistic_predictions_reg <- predict(logisticModel_Reg, newdata = df_test, type = "prob")

roc_curve_reg <- roc(response = df_test$LoanStatus, predictor = logistic_predictions_reg[,1])
...
lines(roc_curve_reg, col="green",xlim = c(1, 0), ylim = c(0, 1))
legend(0.0, 0.5, legend=c("Baseline", "With Interaction", "Regularized"),
       col=c("blue", "red",'green'), lty=1:2, cex=0.8)

text(0.5, 0.8, paste("AUC =", round(auc(roc_curve_noInt), 4)), col = "blue", cex = 1.2)
text(0.5, 0.6, paste("AUC =", round(auc(roc_curve_int), 4)), col = "red", cex = 1.2)
text(0.5, 0.4, paste("AUC =", round(auc(roc_curve_reg), 4)), col = "green", cex = 1.2)
